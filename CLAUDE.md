...
- **已修（2026-08-21）**：`LsParser` 现已支持 ISO `YYYY-MM-DD HH:MM` 与 `Mon DD HH:MM` / `Mon DD YYYY` 两种格式；fixture `ls_la_output_mon.txt` 录自 TCL Android TV 6.0（busybox v1.19.2，`adb exec-out busybox ls -la /`）。另：TCL 默认 `ls` 列布局不标准（无 link-count/size 列）也已修——`reTcl` fallback regex 处理该变体（无 link count、size 可选），fixture `ls_la_output_tcl_default.txt`（`adb shell ls -la /`）覆盖。
