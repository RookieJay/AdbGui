...
/**
 * scrcpy launch panel: install-on-first-use, Start/Stop, collapsible options, recording, and the
 * shortcuts cheat-sheet. Extracted from [DeviceOverviewScreen] so the overview page can host it
 * inside a [SectionCard] without a ~250-line indented block, and so the file stays under the
 * ~300-line guidance.
 *
 * Launch options are session state seeded from the persisted [ScrcpyLaunchProfile]; on Start the
 * profile is re-saved so the last-used options stick across launches.
 */
@Composable
fun ScrcpySection(
    ...
    when (scrcpyStatus.value) {
        "installed" -> {
            ...
            Row(verticalAlignment = Alignment.CenterVertically) {
                ...
                // Stop is the secondary action → outlined, so Start stays the single filled CTA.
